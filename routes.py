...
# async def filter_category_author_limit(category: str, author: str, limit=int, ):
#     _berita = crud.get_category_author_limit(db, category, author, limit)
#     return Response(status="Ok", code="200", message="Success fetch all data", result=_berita)

@router.get("/body_requests")
async def body_request(db: Session = Depends(get_db), title: Optional[str] = None, date: Optional[str] = None, website: Optional[str] = None, category: Optional[str] = None, author: Optional[str] = None):
    _berita = crud.body_request(db, title, date, website, category, author)
    return Response(status="Ok", code="200", message="Success fetch all data", result=_berita)
